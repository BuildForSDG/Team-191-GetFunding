...
from src import db, User, Borrower, Lender
register_bp = Blueprint("reg_bp", __name__, url_prefix="/register")


@register_bp.route("/lender/")
def reg_lender():
    """
...
def is_user_in_db(user_info):
    ...
    email_result = User.query.filter(email=email).first()
    phone = user_info["phone_number"]
    phone_result = User.query.filter(phone_number=phone).first()
